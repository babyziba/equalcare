...
function UploadForm() {
  ...
  return (
    <div className ="uploadForm" onDragOver={(e) => e.preventDefault()} onDrop = {handleDrop} onClick ={handleClick}>
        <input type="file" multiple ref={fileInputRef} style={{display: 'none'}} onChange = {handleChange}/>
        <p>Drag & drop files here or <span className ="uploadBtn">Click to Upload</span> </p>
        {files.length > 0 && (
          <ul className ="fileList"> 
            {files.map((file,idx) => (
              <li key={idx}>{file.name}
                <button className = "removeBtn" onClick={(e) => {e.stopPropagation(0); removeFile(idx);}}>x </button>
              </li>
            ))}
          </ul>
        )}
